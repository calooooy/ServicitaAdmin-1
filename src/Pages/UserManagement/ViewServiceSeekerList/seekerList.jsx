...
function SeekerList({ searchTerm, sortTerm, city, barangay, flagged, onSelectUser, toggleSearchBarVisibility }) {
    ...
    const fetchSeekers = async () => {
      try {
        ...
        await Promise.all(querySnapshot.docs.map(async (doc) => {
          ...
          const response = await Axios.get(`http://192.168.1.7:5000/admin/getUser/${doc.id}`);
  ...
  const handleDelete = (record) => {
    ...
    Axios.post(`http://192.168.1.7:5000/admin/deleteUser`, { userId: record.id })
      .then((response) => {
        const db = getFirestore();
        const seekerCollection = collection(db, "seekers");
  ...
  const handleUnsuspend = (record) => {
    try {
      ...
      Axios.patch('http://192.168.1.7:5000/admin/unsuspendUser', userData)
        .then((response) => {
